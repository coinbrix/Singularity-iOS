...
#ifndef SINGULARITYKIT_SWIFT_H
#define SINGULARITYKIT_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="SingularityKit",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
# pragma pop_macro("any")
#endif

#if defined(__OBJC__)

typedef SWIFT_ENUM(NSInteger, SingularityCustomLoginPlatfrom, open) {
  SingularityCustomLoginPlatfromAuth0 = 0,
  SingularityCustomLoginPlatfromFirebase = 1,
};
...
- (void)onSignatureReceivedWithSignature:(NSString * _Nonnull)signature;
@end
...
+ (void)customLoginWithLoginPlatform:(enum SingularityCustomLoginPlatfrom)loginPlatform accessToken:(NSString * _Nullable)accessToken idToken:(NSString * _Nullable)idToken listener:(id <SingularityListener> _Nonnull)listener;
...
+ (void)requestPersonalSignatureWithMessage:(NSString * _Nonnull)message;
+ (void)requestTypedSignatureWithJsonString:(NSString * _Nonnull)jsonString;
+ (void)openLoginDrawerWithParentVC:(UIViewController * _Nullable)parentVC;
+ (void)initiateTransactionFlowWithRequestString:(NSString * _Nonnull)requestString signature:(NSString * _Nonnull)signature;
